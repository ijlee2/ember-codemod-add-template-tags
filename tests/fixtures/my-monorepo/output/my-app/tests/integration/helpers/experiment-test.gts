...
module('Integration | Helper | experiment', function (hooks) {
  ...
  test('returns true if the experiment name and variant are a match', async function (assert) {
    await render(<template>{{#if (experiment name="experiment-a" variant="v1")}}
    <div data-test-block>
    Some content
    </div>
    {{/if}}</template>);
  ...
  test('returns false if the experiment name and variant are not a match', async function (assert) {
    await render(<template>{{#if (experiment name="experiment-a" variant="control")}}
    <div data-test-block>
    Some content
    </div>
    {{/if}}</template>);
  ...
  test('returns false if the experiment name is unknown', async function (assert) {
    await render(<template>{{! @glint-ignore: We want an error to occur. }}
  ...
  test('returns false if the variant has not been assigned', async function (assert) {
    await render(<template>{{#if (experiment name="experiment-b" variant="v1")}}
    <div data-test-block>
    Some content
    </div>
    {{/if}}</template>);
