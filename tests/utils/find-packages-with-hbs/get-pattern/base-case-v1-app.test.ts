...
test('utils | find-packages-with-hbs | analyze-component > base case (v1 app)', function () {
  const packageType = 'v1-app';
  ...
  assert.deepStrictEqual(output, ['app/components/**/*.hbs']);
  ...
  assert.deepStrictEqual(output, ['app/components/**/index.hbs']);

  output = getPatternForRoutes(packageType, {
    entity,
  });

  assert.deepStrictEqual(output, ['app/templates/**/*.hbs']);
